...
def get_raw_data_opus_books(config):
    ...
    return load_dataset('opus_books', f"{config.DATA['lang_src']}-{config.DATA['lang_tgt']}", split='train')
...
def create_tokenizers_dataloaders(config):
    ...
    train_ds = TwoLanguagesDataset(train_ds_raw, tokenizer_src, tokenizer_tgt, config.DATA['lang_src'], config.DATA['lang_tgt'],
                                   config.DATA['seq_len'])
